...
function Memories({items,query,setQuery,filter,setFilter,catFilter,setCatFilter,open,customImages}){
 ...
 const shelfRows=[0,1,2,3,4];
 const librarySlots=[
  {left:'18%',top:'31%',size:96,delay:0,row:0},{left:'35%',top:'30%',size:72,delay:.2,row:0},{left:'51%',top:'31%',size:78,delay:.4,row:0},{left:'67%',top:'30%',size:84,delay:.6,row:0},
  {left:'24%',top:'44%',size:112,delay:.8,row:1},{left:'45%',top:'43%',size:82,delay:1,row:1},{left:'64%',top:'44%',size:100,delay:1.2,row:1},{left:'79%',top:'43%',size:58,delay:1.4,row:1},
  {left:'15%',top:'57%',size:70,delay:1.6,row:2},{left:'34%',top:'58%',size:98,delay:1.8,row:2},{left:'55%',top:'57%',size:78,delay:2,row:2},{left:'72%',top:'58%',size:88,delay:2.2,row:2},
  {left:'23%',top:'71%',size:84,delay:2.4,row:3},{left:'43%',top:'70%',size:68,delay:2.6,row:3},{left:'61%',top:'72%',size:108,delay:2.8,row:3},{left:'79%',top:'70%',size:64,delay:3,row:3},
  {left:'16%',top:'84%',size:52,delay:3.2,row:4},{left:'36%',top:'84%',size:74,delay:3.4,row:4},{left:'57%',top:'84%',size:58,delay:3.6,row:4},{left:'74%',top:'84%',size:76,delay:3.8,row:4}
 ];
 const memoryPool=filtered.length?filtered:[];
 return <Screen className="library-screen">
  <div className="library-top"><div><h2>我的記憶庫 ✨</h2><p>你收藏的情緒記憶都在這裡</p></div><div className="library-tools"><button><Search size={20}/></button><button><SlidersHorizontal size={19}/></button><button onClick={()=>setFilter('all')}>全部</button></div></div>
  <div className="library-search"><Search size={15}/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="搜尋記憶..."/><select value={catFilter} onChange={e=>setCatFilter(e.target.value)}><option value="all">全部分類</option>{CATEGORIES.map(([c])=><option key={c}>{c}</option>)}</select></div>
  <div className="memory-library-stage">
   <div className="library-portal portal-one"><span>✦</span></div><div className="library-portal portal-two"/><div className="library-sign"><b>記憶庫</b><small>MEMORY LIBRARY</small></div>
   <div className="library-stairs"/><div className="library-rail rail-a"/><div className="library-rail rail-b"/>{shelfRows.map(r=><div key={r} className={`memory-shelf shelf-${r}`}/>) }
   <div className="emotion-side-nav">{EMOTIONS.map((e,i)=><motion.button key={e.key} onClick={()=>setFilter(filter===e.key?'all':e.key)} className={filter===e.key?'active':''} style={{'--emotionColor':e.color}} whileTap={{scale:.92}} animate={{y:[0,-4,0]}} transition={{duration:2.2+i*.18,repeat:Infinity}}>{customImages?.[e.key]?<img src={customImages[e.key]} alt={e.zh}/>:<span>{e.face}</span>}</motion.button>)}</div>
   {memoryPool.length===0?<div className="library-empty"><Sparkles/><b>未有相關記憶</b><p>試試切換其他情緒，或新增第一顆記憶水晶。</p></div>:librarySlots.map((slot,i)=>{const m=memoryPool[i%memoryPool.length];const e=emotionOf(m.emotion);return <motion.button key={`${m.id}-${i}`} className="library-crystal" style={{left:slot.left,top:slot.top,width:slot.size,height:slot.size,'--orb':e.color,'--orbSoft':e.soft}} onClick={()=>open(m)} animate={{y:[0,-8-(i%3)*3,0],scale:[1,1.035,1]}} transition={{duration:4.2+slot.delay,repeat:Infinity,ease:'easeInOut'}}><span>{m.title}</span><small>{customImages?.[e.key]?<img src={customImages[e.key]} alt={e.zh}/>:e.face}</small></motion.button>})}
   <button className="library-add" onClick={()=>window.scrollTo({top:0,behavior:'smooth'})}><Plus size={26}/><span>新增記憶</span></button>
   <div className="library-stat-card"><span>記憶總數</span><b>{items.length}</b><small>今日新增 {todayCount}<br/>最常：{mostEmotion.zh}</small></div>
   <div className="library-recent-card"><span>最近新增</span><b>{recent.title}</b><small>{recent.date}</small></div>
  </div>
 </Screen>}
