...
function Memories({items,query,setQuery,filter,setFilter,catFilter,setCatFilter,open,customImages}){const filtered=items.filter(m=>{const t=(m.title+m.content+m.categories.join('')).toLowerCase();return (filter==='all'||m.emotion===filter)&&(catFilter==='all'||m.categories.includes(catFilter))&&t.includes(query.toLowerCase())});const cosmicSlots=[{left:'18%',top:'21%',size:138,delay:0},{left:'68%',top:'18%',size:102,delay:.45},{left:'70%',top:'48%',size:124,delay:.9},{left:'22%',top:'58%',size:116,delay:1.25},{left:'56%',top:'72%',size:98,delay:1.65},{left:'7%',top:'42%',size:62,delay:2.05},{left:'87%',top:'63%',size:66,delay:2.45},{left:'15%',top:'82%',size:48,delay:2.85},{left:'89%',top:'12%',size:44,delay:3.1},{left:'45%',top:'49%',size:42,delay:3.45}];return <Screen><div className="page-head cosmic-head"><button className="ghost-back"><ChevronLeft size={18}/></button><h2>我的情緒宇宙</h2><Search size={18}/></div><div className="search"><Search size={16}/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="搜尋標題、內容、分類..."/><SlidersHorizontal size={16}/></div><div className="scroll-tabs"><button onClick={()=>setFilter('all')} className={filter==='all'?'active':''}>全部</button>{EMOTIONS.map(e=><button key={e.key} onClick={()=>setFilter(e.key)} className={filter===e.key?'active':''}>{e.zh}</button>)}</div><select className="select" value={catFilter} onChange={e=>setCatFilter(e.target.value)}><option value="all">全部分類</option>{CATEGORIES.map(([c])=><option key={c}>{c}</option>)}</select><div className="universe-area mystery-universe"><div className="cosmic-nebula n-a"/><div className="cosmic-nebula n-b"/><div className="comet-ray"/>{filtered.length===0?<Empty/>:filtered.map((m,i)=>{const e=emotionOf(m.emotion);const slot=cosmicSlots[i%cosmicSlots.length];return <motion.button key={m.id} className={`memory-ball cosmic-ball ${i%5===0?'hero-ball':''}`} style={{...bgStyle(e),left:slot.left,top:slot.top,width:slot.size,height:slot.size}} onClick={()=>open(m)} animate={{y:[0,-16,0],x:[0,(i%2?8:-7),0],scale:[1,1.025,1]}} transition={{duration:4.8+slot.delay,repeat:Infinity,ease:'easeInOut'}}><span>{m.title}</span><small>{customImages?.[e.key]?<img className="ball-custom-face" src={customImages[e.key]} alt={e.zh}/>:e.face}</small></motion.button>})}<button className="floating-plus"><Plus/></button></div></Screen>}
